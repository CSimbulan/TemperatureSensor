...
//    display.display();
    ...
    if (currentMillis - previousMillis2 > saveInterval) {
      ...
      File logFile2 = SD.open("data011c.txt", FILE_WRITE);   // make sure the file name is consistent with the one in VOID SETUP
      if (logFile2)
      {
      ...
      {
        Serial.println("Couldn't open log file in SD card, dammit >_<!");  // If SD card stops working, it will show in serial com
      }
